...
function deletes_(id){
   ...
   if(f){
   location.href="MovieServletdelete?movieName="+id;
   }else{
   alert("您取消删除");
   }
}
...
function update(moviename){
	location.href="updatemovie.jsp?movieName="+moviename;
}
</script>
</head>
<body>
    ...
    <tr>
    <td style="width:70px; background-color:#f7f7f7;">电影名</td>
    <td style="width:70px; background-color:#f7f7f7;">上映时间</td>
    <td style="background-color:#f7f7f7;">制片国家</td>
    <td style="background-color:#f7f7f7;">导演</td>
    <td style="width:80px; background-color:#f7f7f7;">电影类型</td>
    <td style="background-color:#f7f7f7;">编剧</td>
    <td style="width:80px; background-color:#f7f7f7;">总评分</td>
    <td style="width:60px; background-color:#f7f7f7;">评价次数</td>
    <td style="width:30px; background-color:#f7f7f7;">票价</td>
    <td style="background-color:#f7f7f7;">演员</td>
    <td style="background-color:#f7f7f7;">操作</td>
    </tr>
    <%
    //直接调用后台数据
     MovieService ts=new MovieServiceImp();
     List<Movie> list=ts.selects();
  ...
  <td><%=n.getCountry() %></td>
  <td><%=n.getDirector() %></td>
  <td><%=n.getMoviekind() %></td>
  <td><%=n.getScreenwriter() %></td>
  <td><%=n.getGrade() %></td>
  <td><%=n.getNumOfPeople() %></td>
  <td><%=n.getPrice() %></td>
  <td><%=n.getActor() %></td>
